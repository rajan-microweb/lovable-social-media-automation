...
serve(async (req) => {
  ...
  try {
    ...
    if (!short_lived_token || !user_id) {
      ...
      return new Response(JSON.stringify({ error: "short_lived_token and user_id are required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (profileData.error) {
      ...
      return new Response(JSON.stringify({ error: profileData.error.message || "Failed to fetch user profile" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (accountsData.error) {
      ...
      return new Response(JSON.stringify({ error: accountsData.error.message || "Failed to fetch managed pages" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
