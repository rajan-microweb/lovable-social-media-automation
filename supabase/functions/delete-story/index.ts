...
serve(async (req) => {
  ...
  try {
    ...
    if (!story_id || !user_id) {
      console.error('Missing story_id or user_id in request');
      return new Response(
        JSON.stringify({ error: 'story_id and user_id are required' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }

    console.log('Verifying ownership for story:', story_id, 'by user:', user_id);
    ...
    console.log('Deleting story:', story_id);
