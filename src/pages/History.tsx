...
import { RefreshCw, ExternalLink, RotateCcw, Copy, AlertCircle, FileText, CheckCircle2, TrendingUp, XCircle, Trash2, MoreVertical, Instagram, Facebook, Linkedin, Twitter, Search, Calendar, LayoutGrid, List, ChevronLeft, ChevronRight } from "lucide-react";
                                       ...
                                       job.platforms
                                         ...
                                         .filter(p => {
                                            if (effectiveState === 'published') {
                                               const hasAnyLink = job.platforms?.some(plat => getPlatformLink(plat, job.content_metadata, job.content_type, (job as any).url));
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {paginatedJobs.map((job) => {
                      ...
                      return (
                        <Card 
                          key={job.id} 
                          className={`group relative overflow-hidden border transition-all hover:shadow-md ${isSelected ? 'border-primary ring-1 ring-primary/20' : 'border-border/40'}`}
                          onClick={() => toggleSelection(job.id)}
                        >
                          <div className="aspect-square bg-muted/20 relative group-hover:opacity-90 transition-opacity">
                            {previewUrl ? (
                              <img src={previewUrl} alt="" className="h-full w-full object-cover" />
                            ) : (
                              <div className="h-full w-full flex items-center justify-center opacity-20">
                                {job.content_type === "story" ? <FileText className="h-12 w-12" /> : <LayoutGrid className="h-12 w-12" />}
                              </div>
                            )}
                            
                            <div className="absolute top-2 left-2 flex gap-1">
                               <Badge className={`${getContentPipelineStateBadgeClassName(effectiveState)} border-none text-[8px] font-bold uppercase px-1.5 h-4`}>
                                 {effectiveState}
                               </Badge>
                            </div>
                            ...
                            <div className="absolute top-2 right-2">
                               <Checkbox 
                                 checked={isSelected} 
                                 className="rounded-full h-5 w-5 bg-background shadow-sm border-muted-foreground/30 data-[state=checked]:bg-primary data-[state=checked]:border-primary" 
                               />
                            </div>

                            <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent">
                               <p className="text-white text-[10px] font-bold truncate">{job.title || "(No Title)"}</p>
                               <p className="text-white/60 text-[8px] font-medium leading-tight">{format(parseISO(job.run_at), "MMM d, HH:mm")}</p>
                            </div>
                          </div>

                          <div className="p-3 flex items-center justify-between border-t border-border/5">
                            <div className="flex gap-1">
                                {job.platforms?.filter(Boolean).map((p, idx) => {
                                  const link = getPlatformLink(p, job.content_metadata, job.content_type, (job as any).url);
                                  return (
                                    <div key={idx} className="h-5 w-5">
                                      <PlatformIcon platform={p} link={link} />
                                    </div>
                                  );
                                })}
                            </div>
                            ...
                            <DropdownMenu>
                               ...
                               <DropdownMenuContent align="end" className="w-40">
                                 ...
                                 >
                                    <ExternalLink className="h-3.5 w-3.5 mr-2 opacity-60" /> Edit Detail
                                 </DropdownMenuItem>
                                 <DropdownMenuItem className="text-destructive font-bold text-xs" onClick={async () => {
                                    if (window.confirm("Remove log?")) {
                                       await supabase.from("publish_jobs").delete().eq("id", job.id);
                                       await refreshJobs();
                                    }
                                 }}>
                                    <Trash2 className="h-3.5 w-3.5 mr-2" /> Delete Log
                                 </DropdownMenuItem>
                               </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
